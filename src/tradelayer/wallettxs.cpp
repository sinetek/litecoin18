...
static int64_t GetEstimatedFeePerKb()
{
    ...
    CWalletRef pwalletMain = nullptr;
    if (vpwallets.size() > 0){
        pwalletMain = vpwallets[0].get();
    }

    if (pwalletMain) {
       CCoinControl coin_control;
       nFee = GetMinimumFee(*pwalletMain, 1000, coin_control, mempool, ::feeEstimator, nullptr);
    }
#endif
    return nFee;
}
...
int IsMyAddress(const std::string& address)
{
    ...
    if (pwalletMain) {
	    auto isMine = IsMine(*pwalletMain, DecodeDestination(address));
		...
		unsigned int minOutputs)
{
	...
	for (auto& map : pwalletMain->mapWallet)
	{
		...
		if (ctx.GetAvailableCredit(*locked_chain)) {
			continue;
		}
